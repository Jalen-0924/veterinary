...
class Invoice extends BaseController
{
   ...
   public function index(){
    ...
    // Fetching patient data directly from the user model.
    $patientData = $userModel->getUsers();
    $printData = $Model->print_invoice($id);

    // Check if 'patient' data exists and is structured as expected
    if (isset($printData['patient'])) {
        $printData['patient'] = $patientData; // If you need to map patient data to 'print'
    } else {
        $printData['patient'] = []; // Set as an empty array if 'patient' doesn't exist to avoid errors
    }

    $data['print'] = $printData;

    // Optional: Print and debug the data structure
    // print_r($data['print']);
    // die();
