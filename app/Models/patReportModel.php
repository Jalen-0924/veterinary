...
class patReportModel extends Model{
   ...
   public function get_patient_report_by_id($id){
    ...
    $doctor = $userModel->where('id', $doctor_id)->find();
    if (empty($doctor)) {
        return null; // or handle this case appropriately
    }
    $doctor = $doctor[0];
    ...
    $patient = $userModel->where('id', $patient_id)->find();
    if (empty($patient)) {
        return null; // or handle this case appropriately
    }
    $patient = $patient[0];
    ...
    $pet = $petModel->where('id', $pet_id)->find();
    if (empty($pet)) {
        return null; // or handle this case appropriately
    }
    $pet = $pet[0];
    ...
    $appointment = $appointmentModel->where('id', $app_id)->find();
    ...
    if (!empty($service_ids)) {
        foreach ($service_ids as $id) {
            $service = $serviceModel->where('id', $id)->find();
            if (!empty($service)) {
    ...
    if (!empty($medication_ids)) {
        foreach ($medication_ids as $id) {
            $medicine = $medicineModel->where('id', $id)->find();
            if (!empty($medicine)) {
                $medicines[] = $medicine[0]['name'];
            }
        }
    }
